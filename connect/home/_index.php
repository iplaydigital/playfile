<?php error_reporting(E_ALL ^ E_NOTICE);
/*****************************************************************
Created :28/02/2565
Author : worapot pilabut (aj.ake)
E-mail : worapot.playdigital@gmail.com
Website : https://conenct.playdigital.co.th
Copyright (C) 2023, Play digital Co.,Ltd. all rights reserved.
 *****************************************************************/


include_once("../include/config.inc.php");
include_once("../include/class.inc.php");
include_once("../include/class.TemplatePower.inc.php");
include_once("../include/function.inc.php");
include_once("../authentication/check_login.php");



$tpl = new TemplatePower("../template/_tp_inner.html");
$tpl->assignInclude("body", "_tp_index.html");
$tpl->prepare();
$tpl->assign("_ROOT.page_title", "หน้าแรก");
$tpl->assign("_ROOT.logo_brand_alt", $Brand);
$tpl->assign("_ROOT.fullname",$_SESSION['FULLNAME']);



$TodayThaiShow = ThaiToday($strDateTime, $tnow);

/*
$query	= "SELECT * FROM `tb_task_all` WHERE `STATUS`='Show' ORDER BY `ID` DESC";
$result	= mysql_query($query);
while($line = mysql_fetch_array($result)) {
$tpl->newBlock("WORKLIST");



$tpl->assign("img",$line["THUMB"]);




}
*/

////////////////////////////////////////////////////////////////////////////////////////
$query1 = '{boards {workspace {id name}}}';

 $data1 = @file_get_contents($mondayapiUrl, false, stream_context_create([
    'http' => [
        'method' => 'POST',
        'header' => $mondayheaders,
        'content' => json_encode(['query' => $query1]),
    ]
]));
$array1 = json_decode($data1, true);
foreach ($array1['data']['workspace'] as $workspace) {

    $tpl->newBlock("WORKSPACE");
    $tpl->assign("id",$workspace['id']);
    $tpl->assign("name","".$workspace['name']."");
}

////////////////////////////////////////////////////////////////////////////////////////



$query2 = '{ boards { id name } }';
$data2 = @file_get_contents($mondayapiUrl, false, stream_context_create([
    'http' => [
        'method' => 'POST',
        'header' => $mondayheaders,
        'content' => json_encode(['query' => $query2]),
    ]
]));
$array2 = json_decode($data2, true);
foreach ($array2['data']['boards'] as $board) {

    $tpl->newBlock("WORKLIST");
    $tpl->assign("id",$board['id']);
    $tpl->assign("title","".$board['name']."");
}













$tpl->assign("_ROOT.Powerby", $Powerby);
$tpl->assign("_ROOT.Copyright", $Copyright);
$tpl->printToScreen();

