<?
	...
	if($_POST['submit']){
		...
		$rrs = mysql_query("select * from album where category_id='".$_GET['fid']."'");
		while($rrw = mysql_fetch_array($rrs)){
			$sorder = $rrw['album_order']+1;
			mysql_query("update album set album_order='".$sorder."' where album_id='".$rrw['album_id']."'");
		}
		
		mysql_query("insert into album(category_id, album_title, album_short_description, album_status, album_createdtime, album_createdip) values('".$_GET['fid']."', '".$_POST['album_title']."', '".$_POST['album_short_description']."', '".$_POST['status']."', '".$today."', '".$_SERVER['REMOTE_ADDR']."')");
		...
		if($pics1){
			...
			if($last_name1=="gif" or $last_name1=="jpg" or $last_name1=="jpeg" or $last_name1=="png"){
				copy($pics1, "../public/food/album/".$fir1.$id.".".$last_name1);
			}
			$p.="album_cover='".$fir1.$id.".".$last_name1."', ";			
		}
		
		mysql_query("update album set $p album_createdtime='".$today."' where album_id='".$id."'");
	...
	<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="icon-edit"></i><span class="break"></span>เพิ่มรูปภาพ</h2>
						<!--<div class="box-icon">
