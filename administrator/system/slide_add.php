<?php
	...
	if($_POST['submit']){
		...
		$rrs = mysql_query("select * from slide");
		while($rrw = mysql_fetch_array($rrs)){
			$sorder = $rrw['sorder']+1;
			mysql_query("update slide set sorder='".$sorder."' where slide_id='".$rrw['slide_id']."'");
		}
			if($pics1){
				mysql_query("insert into `slide`(url, youtube, type, sorder, status) values('".$_POST['url']."', '".$_POST['youtube']."', '".$_POST['type']."', '1', 'approved')");
				$id = mysql_insert_id();
				...
				if($last_name1=="gif" or $last_name1=="jpg" or $last_name1=="jpeg" or $last_name1=="png"){
					copy($pics1, "../public/slide/s".$id.".".$last_name1);
				}
				
				mysql_query("update slide set pics='s".$id.".".$last_name1."' where slide_id='".$id."'");
			}else{ 
				mysql_query("insert into `slide`(youtube, type, sorder, status, pics) values('".$_POST['youtube']."', '".$_POST['type']."', '1', 'approved', 'default.jpg')");
			}
...
<script>
	function changeoption(id){
		window.location='index.php?page=slide_add&mid='+id;
	}
</script>
