<?
	...
	if($_POST['submit']){
		...
		mysql_query("insert into content(product_id, f_type_id, category_id, content_title_en, content_title_th, content_sdesc_en, content_sdesc_th, content_desc_en, content_desc_th, content_status, content_createdip) values('".$_GET['rid']."', '".$_POST['f_type_id']."', '1', '".$content_title_en."', '".$content_title_th."', '".$content_sdesc_en."', '".$content_sdesc_th."', '".$content_desc_en."', '".$content_desc_th."', '".$_POST['content_status']."', '".$_SERVER['REMOTE_ADDR']."')");
	...
	<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="icon-edit"></i><span class="break"></span>สร้างหมวดหมู่ตำรับความอร่อย</h2>
						<!--<div class="box-icon">
							  ...
							  <div class="control-group">
								...
								<div class="controls">
								  <select id="f_type_id" name="f_type_id">
									...
									while($row = mysql_fetch_array($res)){
									?>
									<option value='<?=$row['f_type_id']?>' ><?=$row['f_type_name_th']?></option>
									<? }?>
								  </select>
								</div>
							  </div>
